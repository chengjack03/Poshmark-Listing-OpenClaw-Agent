...
def generate_listing(photo_paths, group_description):
    ...
    # Normalize None values from llama before any processing
    data["brand"]      = data.get("brand")      or "Unknown"
    data["size"]       = data.get("size")        or "One Size"
    data["color"]      = data.get("color")       or "Unknown"
    data["condition"]  = data.get("condition")   or "Gently Used"
    data["title"]      = data.get("title")       or "Kids Clothing Item"
    data["list_price"] = data.get("list_price")  or "10"
...
def send_telegram_notification(results):
    """Send a Telegram summary of drafted listings via Bot API."""
    import requests
    ...
    if count == 0:
        ...
        if failed:
            msg += f" ({failed} failed)"
    print(f"\n  📲 Sending Telegram notification to chat {TELEGRAM_CHAT_ID}...")
    try:
        resp = requests.post(
